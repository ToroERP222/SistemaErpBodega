...
const res = await axios.put(`${process.env.IP}/api/v1/productos/actualizarAlmacen/${p._id}`,data)
console.log(res)
}
...
const PedidoFabricante =  () => {
  ...
  const handlesubmit = async(d) => {
    ...
    console.log(data)
    const resp = await axios.post(`${process.env.IP}/api/v1/almacen/pedidoF/crear`,data)
  }
  return(
    <>
        <div>
...
let namesP = [{nombre:'',img:''}]
 ...
 console.log(formData.getAll('img'))
  const config = {
   ...
   const resp = await axios.post(`${process.env.IP}/api/v1/almacen/estatusA/crear`,formData,config)
